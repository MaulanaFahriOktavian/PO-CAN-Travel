...
<div class="py-10 sm:py-12">
    <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="mb-8">
            <a href="{{ route('admin.trips.index') }}" class="text-sm font-medium text-blue-600 hover:text-blue-800 transition-colors">
                &larr; Kembali ke Daftar Jadwal
            </a>
            <h1 class="text-2xl font-bold text-slate-900 tracking-tight mt-2">Tambah Jadwal Perjalanan</h1>
            <p class="mt-1 text-sm text-slate-600">Jadwalkan perjalanan baru dengan memilih rute, armada bus, dan penetapan tarif.</p>
        </div>

        <div class="bg-white border border-slate-200 rounded-xl p-6 sm:p-8 shadow-sm">
            <form method="POST" action="{{ route('admin.trips.store') }}" class="space-y-5">
                @csrf
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        <label for="departure_at" class="block text-sm font-medium text-slate-700 mb-1">Waktu Keberangkatan</label>
                        <input
                            type="datetime-local"
                            id="departure_at"
                            name="departure_at"
                    ...
                    <div>
                        <label for="arrival_at" class="block text-sm font-medium text-slate-700 mb-1">Waktu Kedatangan</label>
                        <input
                            type="datetime-local"
                            id="arrival_at"
                            name="arrival_at"
                ...
                <div>
                    <label for="price" class="block text-sm font-medium text-slate-700 mb-1">Tarif Tiket (Rupiah)</label>
                    <input
                        type="number"
                        id="price"
                        name="price"
                        value="{{ old('price', 250000) }}"
                        min="0"
                        step="1000"
                        required
