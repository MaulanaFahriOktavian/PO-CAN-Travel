        ...
        <div class="bg-white border border-slate-200 rounded-xl p-5 mb-6 space-y-4">
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                ...
                <div class="flex flex-wrap gap-1.5 text-xs font-medium">
                    <a
                        href="{{ route('admin.orders.index', array_filter(['search' => $search])) }}"
                    ...
                    <a
                        href="{{ route('admin.orders.index', array_filter(['status' => 'pending', 'search' => $search])) }}"
                    ...
                    <a
                        href="{{ route('admin.orders.index', array_filter(['status' => 'confirmed', 'search' => $search])) }}"
                    ...
                    <a
                        href="{{ route('admin.orders.index', array_filter(['status' => 'completed', 'search' => $search])) }}"
                    ...
                    <a
                        href="{{ route('admin.orders.index', array_filter(['status' => 'cancelled', 'search' => $search])) }}"
                            ...
                            <tr class="hover:bg-slate-50/75 transition-colors">
                                ...
                                <td class="px-5 py-4 text-center whitespace-nowrap">
                                    @if ($order->status === 'pending')
                                        <span class="inline-block text-xs font-medium text-amber-800 bg-amber-50 px-2.5 py-1 rounded border border-amber-200">
                                            Menunggu Pembayaran
                                        </span>
                                    @elseif ($order->status === 'confirmed')
                                        <span class="inline-block text-xs font-medium text-blue-800 bg-blue-50 px-2.5 py-1 rounded border border-blue-200">
                                            Dikonfirmasi
                                        </span>
                                    ...
                                    @elseif ($order->status === 'cancelled')
                                        <span class="inline-block text-xs font-medium text-rose-800 bg-rose-50 px-2.5 py-1 rounded border border-rose-200">
                                            Dibatalkan
                                        </span>
                                    @else
                                        <span class="inline-block text-xs font-medium text-slate-800 bg-slate-100 px-2.5 py-1 rounded border border-slate-200 capitalize">
                                            {{ $order->status }}
                                        </span>
                                    @endif
                                </td>
                        ...
                        @empty
                            <tr>
                                <td colspan="8" class="px-5 py-12 text-center text-slate-500">
                                    ...
                                    @if ($search || $selectedStatus)
                                        <p class="text-xs text-slate-500 mt-1">Coba sesuaikan kata kunci pencarian atau filter status Anda.</p>
                                    @endif
                                </td>
                            </tr>
                        @endforelse
